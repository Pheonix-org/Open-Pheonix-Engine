...
/**
 * Main thread management tool for JGEL. Creates, executes, stores, manages and
 * disposes of threads.
 *
...
public class JGELThreadManager implements JGELHook {
	...
	public static List<JGELThread> getAllThreads() {
		List<JGELThread> copy = new ArrayList<>();
		copy.addAll(threads);
		return copy;
	}
	...
	/**
	 * Invokes the JGELRunnable ThreadStop method on a thread. This method requests
	...
	public static void invokeThreadStop(JGELThread thread) throws JGELThreadPersistance {
		try {
			...
			EMSHelper.handleException(
					new JGELThreadPersistance(thread, "an exception was thrown in the thread's stop method."));
		}
	}
