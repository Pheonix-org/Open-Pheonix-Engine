...
public class EMSHelper {
	...
	/**
	 * Main exception handler for JGEL. All internal threads use this handler. This
	 * should be the main way for the JGELEMS to be called in an error state.
	 */
	...
	public static void handleException(Throwable e, Boolean Silent) {
		...
		if (AllowErrNotif) { // If notification is on
			...
			// Show notifiction
			JOptionPane.showMessageDialog(JGELWindowHelper.getSwingParent(),
					"JGEL has encountered an error. Data will be backed up. Further problems may occour.");
			if (Silent) {
	...
	private static void detectCascade() {
		...
		if (CascadeCount > CascadeTollerance) { // If cascade tollerance is exceeded
			if (AllowEIS) { // Check EIS perms
				...
				JOptionPane.showMessageDialog(JGELWindowHelper.getSwingParent(),
						"A cascade of errors has been detected, but JGEL has been instructed to not shutdown.");
				JOptionPane.showMessageDialog(JGELWindowHelper.getSwingParent(),
	...
	public static void invokeEIS() {
		...
		JOptionPane.showMessageDialog(JGELWindowHelper.getSwingParent(),
				"JGEL Runtime is experiencing problems and is closing.");
