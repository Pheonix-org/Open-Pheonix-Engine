...
import backend.io.JGELFileUtils;
...
public class CommandBuilder {
	...
	private void initialize() {
		...
		frmJgelCommandBuilder.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
		...
		btnLoadJgelcmd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				FileNameExtensionFilter filter = new FileNameExtensionFilter("JGEL Instruction File", "jgelcmd");
				chooser.setFileFilter(filter);
				int returnVal = chooser.showOpenDialog(null);
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					try {
						Instruction = JGELFileUtils.<InstructionBase>deserialize(chooser.getSelectedFile(),
								Instruction);
	...
	protected void BuildClass() {
		...
		Builder parseBuilder = MethodSpec.methodBuilder("parse").addAnnotation(Override.class)
				...
				.beginControlFlow("switch (JGELConsole.getParamString(\"Switch to perform\", ConsoleOptions.class))");
		...
		Builder HelpBuilder = MethodSpec.methodBuilder("help").addAnnotation(Override.class)
				.addModifiers(Modifier.PUBLIC).addStatement("JGELConsole.Write(\"Arguments:\")");

		// populate switches
		for (Switch s : Instruction.switches) {
			HelpBuilder.addStatement("JGELConsole.Write(\"" + s.name + " - " + s.description + "\")");
