...
//			}
	...
	public static void writeOut() {
			...
			try {
				...
				for (String log : Logs) { // For every log,
					if (log == null || log.equals("null")) {
						continue;
					} // (Ignore null or empty logs)
					writer.append(System.lineSeparator() + log); // append the log to the file on a new line.
				}

				// CLOSE RESOURCES
				writer.append(System.lineSeparator() + System.lineSeparator()); // WS on file's end to separate call
																				// stack
		...
		JOptionPane.showMessageDialog(JGELWindowHelper.getSwingParent(),
				"Log may not have been created, use the terminal.");
		EMSHelper.warn("");
	}
