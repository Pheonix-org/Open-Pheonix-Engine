...
public class FilesHelper {
	...
	public static boolean WriteCJAR(File target, CJARMeta meta) {
			ArrayList<File> files = new ArrayList<>();
			   for (String pathToJar : meta.paths()) {
				File tobeJared = new File(target + pathToJar);
				files.add(tobeJared);
			}
	...
	public static boolean WriteJAR(File target, ArrayList<File> files) {
		try {
			...
			File jar = new File(target.getAbsolutePath() + ".cjar");
			if (!jar.exists()) {
				jar.createNewFile();
			} else {
				EMSHelper.warn("archive already exists! Aborting jar write.");
			}
			FileOutputStream stream = new FileOutputStream(jar.getAbsolutePath());
			JarOutputStream out = new JarOutputStream(stream, manifest);

			for (File file : files) {
				if (!file.exists()) continue; 																			// Just in case...
				try {
    ...
    public static InputStream getFromResources(String path) {
		return FilesHelper.class.getResourceAsStream(path);
    }
