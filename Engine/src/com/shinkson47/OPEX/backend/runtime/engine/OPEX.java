...
public class OPEX implements IOPEXRunnable, IOPEXVersionable {
	...
	public OPEX(OPEXGame game) throws OPEXStartFailure {
		try {
			 ...
			 */
			isStarting = true;
			OPEXConsole.internalLog("Valid startup call for OPEX backend framework was issued, starting up. Won't be a jiffy!");//Log valid startup call
	...
	public void run() {
		 ...
		 */
		OPEXConsole.internalLog("Runtime thread started.");																//Log successful launch of OPEX startup thread.
		 ...
		 */
		OPEXConsole.internalLog("Internal Framework startup routine has finished invoking subroutines. Subroutines may still be processing. ");
		...
		OPEXConsole.internalLog("OPEX startup routine has completed, waiting for splash screen to close.");				//Log completion, and wait
		while (Splash.isSplashVisible()){
			try {Thread.sleep(100);
			} catch (InterruptedException e) {}}
		waitForStartup = false;
		OPEXConsole.internalLog("Done, executing post start and game payload.");										//Log switch to game runnable.
